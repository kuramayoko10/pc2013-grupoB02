...
>>>>>>> Ordenacao das atividades de cada node MPI
...
=======
    ...
    if(rc == MPI_SUCCESS)
    {
        ...
        if (rank == 0) {
            ...
            for(count=0; count < NUMBER_OF_NODES; count++)
                outmsg[count] = (char*) malloc (SIZE_OF_SEGMENT*sizeof(char ));
            //outmsg = (char**)malloc(NUMBER_OF_NODES * sizeof(char*));
            
            for(count=0; count < NUMBER_OF_NODES; count++)
            {
                //outmsg[count] = (char*)malloc(sizeof(char)*SIZE_OF_SEGMENT);
                outmsg[count][0] = '\0';
            }
            ...
            for(dest = 1; dest < NUMBER_OF_NODES; dest++)
                rc = MPI_Send((char*)outmsg[dest-1], sizeOfVector[dest-1], MPI_CHAR, dest, tag, MPI_COMM_WORLD);
        }
        else{
            source = 0;		
            rc = MPI_Recv(&inmsg, sizeOfVector[rank-1], MPI_CHAR, source, tag, MPI_COMM_WORLD, &Stat);
